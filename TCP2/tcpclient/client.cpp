...
void Client::openFile()
{
    ...
    if (!fileName.isEmpty()) {
        ...
        ui->clientStatusLabel->setText(QStringLiteral("打开文件 %1 成功！").arg(fileName));
    }
}
...
void Client::send()
{
    ...
    // 初始化已发送字节为0
    bytesWritten = 0;
    ui->clientStatusLabel->setText(QStringLiteral("连接中…"));
...
//    qDebug() << fileName.right(fileName.size() - fileName.lastIndexOf('/'));
...
void Client::updateClientProgress(qint64 numBytes)
{
    ...
    if(bytesWritten == totalBytes) {
        ui->clientStatusLabel->setText(QStringLiteral("传送文件 %1 成功").arg(fileName));
        localFile->close();
        tcpClient->close();
    }
}
...
void Client::displayError(QAbstractSocket::SocketError)
{
    ...
    ui->clientStatusLabel->setText(QStringLiteral("客户端就绪"));
    ui->sendButton->setEnabled(true);
}
...
{
    ui->clientProgressBar->reset();
    ui->clientStatusLabel->setText(QStringLiteral("状态：等待打开文件！"));
    openFile();
