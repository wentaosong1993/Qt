...
void MainWindow::ftpCommandStarted(int)
{
    ...
    case QFtp::ConnectToHost :
        ui->label->setText(QStringLiteral("正在连接到服务器…"));
        break;
    case QFtp::Login :
        ui->label->setText(QStringLiteral("正在登录…"));
        break;
    case QFtp::Get :
        ui->label->setText(QStringLiteral("正在下载…"));
        break;
    case QFtp::Close :
        ui->label->setText(QStringLiteral("正在关闭连接…"));
    }
}
...
void MainWindow::ftpCommandFinished(int, bool error)
{
    if(ftp->currentCommand() == QFtp::ConnectToHost) {
        if (error)
            ui->label->setText(QStringLiteral("连接服务器出现错误：%1").arg(ftp->errorString()));
        else ui->label->setText(QStringLiteral("连接到服务器成功"));
    } else if (ftp->currentCommand() == QFtp::Login) {
        if (error)
            ui->label->setText(QStringLiteral("登录出现错误：%1").arg(ftp->errorString()));
        else {
        ...
        if(error)
            ui->label->setText(QStringLiteral("下载出现错误：%1").arg(ftp->errorString()));
        else {
            ui->label->setText(QStringLiteral("已经完成下载"));
            file->close();
        }
        ...
        if (isDirectory.isEmpty())
        {
            ui->fileList->addTopLevelItem(
                        new QTreeWidgetItem(QStringList()<< QStringLiteral("<empty>")));
            ui->fileList->setEnabled(false);
            ui->label->setText(tr("该目录为空"));
        }
    } else if (ftp->currentCommand() == QFtp::Close) {
        ui->label->setText(QStringLiteral("已经关闭连接"));
    }
}
