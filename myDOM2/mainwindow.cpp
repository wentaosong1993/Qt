    ...
    ui(new Ui::MainWindow)
{
    ...
    QDomElement title = doc.createElement(QStringLiteral("书名"));
    ...
    title = doc.createElement(QStringLiteral("书名"));
...
{
    ui->listWidget->clear();
    ui->listWidget->addItem(QStringLiteral("没有找到相关内容！"));
    QFile file("my.xml");
    ...
    for(int i=0; i<list.count(); i++)
    {
        ...
        if(e.attribute(QStringLiteral("编号")) == ui->lineEdit->text())
    ...
    while(!n.isNull())
    {
        if (n.isElement())
        {
            ...
            for (int i=0; i<list.count(); i++)
            {
                QDomNode node = list.at(i);
                if(node.isElement())
                    ui->listWidget->addItem(QStringLiteral("   ") + node.toElement().tagName()
                                            + QStringLiteral(" : ") + node.toElement().text());
            }
        }
        n = n.nextSibling();
    }
...
void MainWindow::on_pushButton_4_clicked()
{
    ...
    QString num = root.lastChild().toElement().attribute(QStringLiteral("编号"));
