...
JPShadow::pfnUpdateLayeredWindow JPShadow::s_UpdateLayeredWindow = NULL;
HINSTANCE JPShadow::s_hInstance = (HINSTANCE)INVALID_HANDLE_VALUE;
...
JPValArray JPShadow::s_ShadowArray(sizeof(HwndShadow), 10);

JPValArray::JPValArray(int iElementSize, int iPreallocSize):
    m_pVoid(nullptr),
    m_nCount(0),
...
void JPValArray::Empty()
{
    m_nCount = 0;
}

bool JPValArray::IsEmpty() const
{
    return m_nCount == 0;
}

bool JPValArray::Add(LPCVOID pData)
{
...
bool JPValArray::Remove(int iIndex, int iCount)
{
...
int JPValArray::GetSize() const
{
    return m_nCount;
}

LPVOID JPValArray::GetData()
{
    return static_cast<LPVOID>(m_pVoid);
}

LPVOID JPValArray::GetAt(int iIndex) const
{
...
LPVOID JPValArray::operator[](int nIndex) const
{
...
bool JPShadow::Initialize(HINSTANCE hInstance)
{
    ...
    wcex.lpfnWndProc = JPShadow::WndProc;
    wcex.cbClsExtra = 0;
    wcex.cbWndExtra = 0;
...
HWND JPShadow::GetHWND() const
{
    return m_hWnd;
}

JPShadow::operator HWND() const
{
    return m_hWnd;
}

void JPShadow::Create(HWND hParentWnd)
{
    if (!s_UpdateLayeredWindow)
        return;
...
bool JPShadow::SetImage(LPCTSTR image, RECT rcCorner, RECT rcHoleOffset)
{
...
bool JPShadow::SetSize(int NewSize)
{
...
bool JPShadow::SetSharpness(unsigned int NewSharpness)
{
    if (NewSharpness > 20)
        return false;
...
bool JPShadow::SetDarkness(unsigned int NewDarkness)
{
    if (NewDarkness > 255)
        return false;
...
bool JPShadow::SetPosition(int NewXOffset, int NewYOffset)
{
...
bool JPShadow::SetColor(COLORREF NewColor)
{
    m_Color = NewColor;
    if (SS_VISABLE & m_Status)
        Update(GetParent(m_hWnd));
...
LRESULT JPShadow::WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    JPShadow* pThis = NULL;
...
LRESULT JPShadow::ParentProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    JPShadow *pThis = FindShadowWindow(hWnd);
    _ASSERT(pThis);
...
JPShadow * JPShadow::FindShadowWindow(HWND hWnd)
{
...
int JPShadow::GetShadowWindowIndex(HWND hWnd)
{
    for (int i = 0; i < s_ShadowArray.GetSize(); ++i)
    {
        HwndShadow* hwndShadow = (HwndShadow*)s_ShadowArray[i];
...
void JPShadow::Update(HWND hParent)
{
    //int ShadSize = 5;
...
void JPShadow::Update()
{
    Update(GetParent(m_hWnd));
}

void JPShadow::MakeShadow(UINT32 * pShadBits, HWND hParent, RECT * rcParent)
{
